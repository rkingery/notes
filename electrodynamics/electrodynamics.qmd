...
$$
The stress tensor evidently has dimensions of energy density, or equivalently dimensions of *pressure*, carrying units of $\text{dyne}/\text{cm}^2$ in Gaussian units or $\text{N}/\text{m}^2$ in SI units. Since $\mathbf{T}$ is a linear combination of valid rank-2 tensors, it must indeed be a valid rank-2 tensor as we claimed. Moreover, $\mathbf{T}$ is a symmetric tensor since $T_{ij} = T_{ji}$.

Notice that the divergence of a rank-2 tensor is evidently a vector since we contract over only one of the indices. Since $\mathbf{T}$ is symmetric we have $\partial_j T_{ij} = \partial_i T_{ij}$, which means we can take the divergence by contracting over either index, meaning the notation $\nabla \cdot \mathbf{T}$ is unambiguous.

The fact that the stress tensor has dimensions of pressure suggests that not only do EM fields have energy and momentum, but they also have pressure. These pressures are given by the diagonals of the stress tensor. The off-diagonal components of the stress tensor represent shearing.

The closed surface integral of the stress tensor represents the total force flowing out of a given fixed volume,
$$
\mathbf{F} = \oint_\mathcal{S} \mathbf{T} \cdot d\mathbf{a} \ .
$$
We've thus shown that the conservation law for field momentum is given by
$$
\boxed{
...
- Talk more about EM pressure, maybe illustrate with a simple example or figure.
- Consider working an example, e.g. Coulomb's law the hard way
- Why does $\nabla \cdot \mathbf{T}$ carry a minus sign in the conservation law?

$$
T_{xx} = \frac{1}{4\pi} \bigg[E_x^2 + B_x^2 - \frac{1}{2}(E_k E_k + B_k B_k)\bigg] = \frac{1}{8\pi} \bigg[(E_x^2 - E_y^2 - E_z^2) + (B_x^2 - B_y^2 - B_z^2)\bigg]
$$
