...
$$
In this limit the equipotentials propagate outward from the moving charge as spherical waves moving at the speed of light. However, when $|\mathbf{v}_p| \sim c$ these waves are no longer spherical. In this limit, we have two cases. In the plane normal to the charge's motion, $\mathcal{R} = 1$ at all speeds, which means the field lines in this plane still move outward as circular waves. However, outside this normal plane the equipotentials get *contracted* near the moving charge since $\mathcal{R} \to \infty$.
...
Let's now turn our attention to conservation of EM field momentum. It may be surprising that EM fields contain momentum at all. After all, in classical mechanics we defined momentum as the product of mass and velocity, yet we've not said anything about whether EM fields have mass (they don't). Nevertheless, that EM fields have momentum is a necessarily consequence of the Lorentz force law.
...
$$
Inside the integrand we again see a density, this time the *force density* $\mathbf{f}$ on the charge distribution due to the EM field,
$$
\mathbf{f}(\mathbf{x},t) \equiv \rho(\mathbf{x},t) \mathbf{E}(\mathbf{x},t) + \frac{\mathbf{J}(\mathbf{x},t)}{c} \times \mathbf{B}(\mathbf{x},t) \ .
$$
Now, recall that the force is by definition the time derivative of *mechanical momentum*,
$$
\mathbf{F} = \frac{d\mathbf{P}_\text{mech}}{dt} \ .
$$
This means if we pull the derivative inside the above integral then we can think of the force density as the partial time derivative of a mechanical *momentum density* $\mathbf{g}_\text{mech}$. This means we have
$$
\frac{\partial\mathbf{g}_\text{mech}}{\partial t} = \mathbf{f} = \rho \mathbf{E} + \frac{\mathbf{J}}{c} \times \mathbf{B} \ .
$$
Now we'll apply Maxwell's equations to convert the densities into fields to get
$$
\begin{align*}
\frac{\partial\mathbf{g}_\text{mech}}{\partial t} &= \frac{1}{4\pi} (\nabla \cdot \mathbf{E}) \mathbf{E} + \frac{1}{c} \bigg(\frac{c}{4\pi}\nabla \times \mathbf{B} - \frac{1}{4\pi} \frac{\partial \mathbf{E}}{\partial t} \bigg) \times \mathbf{B} \\
&= \frac{1}{4\pi} \bigg[(\nabla \cdot \mathbf{E}) \mathbf{E} + (\nabla \times \mathbf{B}) \times \mathbf{B} - \frac{1}{c} \frac{\partial \mathbf{E}}{\partial t} \times \mathbf{B} \bigg]
\end{align*}
$$
Next, observe by applying both the product rule and Faraday's law we can write
$$
...
$$
Now, since $\nabla \cdot \mathbf{B} = 0$ we can add any function of $\nabla \cdot \mathbf{B}$ to the force density without changing its value. If we add $(\nabla \cdot \mathbf{B})\mathbf{B}$ to the inside of the brackets above and insert the above product rule identity, we can thus write
$$
\frac{\partial\mathbf{g}_\text{mech}}{\partial t} = \frac{1}{4\pi} \big[(\nabla \cdot \mathbf{E}) \mathbf{E} + (\nabla \cdot \mathbf{B})\mathbf{B} - \mathbf{B} \times (\nabla \times \mathbf{B}) - \mathbf{E} \times (\nabla \times \mathbf{E})\big] - \frac{1}{4\pi c} \frac{\partial}{\partial t} (\mathbf{E} \times \mathbf{B}) \ .
$$
Moving the time derivative of $\mathbf{E} \times \mathbf{B}$ to the left-hand side, this then becomes
$$
\frac{\partial\mathbf{g}_\text{mech}}{\partial t} + \frac{\partial}{\partial t} \bigg(\frac{\mathbf{E} \times \mathbf{B}}{4\pi c}\bigg) = \frac{1}{4\pi} \big[(\nabla \cdot \mathbf{E}) \mathbf{E} + (\nabla \cdot \mathbf{B})\mathbf{B} - \mathbf{B} \times (\nabla \times \mathbf{B}) - \mathbf{E} \times (\nabla \times \mathbf{E})\big] \ .
$$
On the left-hand side we now have a partial derivative of the mechanical momentum density $\mathbf{g}_\text{mech}$ plus some other momentum density $\mathbf{g}$, which we interpret to be the density of momentum stored in the EM fields themselves,
$$
\boxed{
\mathbf{g} \equiv \frac{1}{4\pi c} \mathbf{E} \times \mathbf{B}
} \ .
$$
Notice that the momentum density is proportional to the Poynting vector, with
$$
\mathbf{g} = \frac{\mathbf{S}}{c^2} \ .
$$
This means the EM field momentum will move in the same direction as the energy, both perpendicular to both $\mathbf{E}$ and $\mathbf{B}$. The dimensions of momentum density are clearly momentum per unit volume, or $\text{erg} \cdot \text{s} / \text{cm}^3$ in Gaussian units.

The total momentum $\mathbf{P}_\text{EM}$ stored in the fields is evidently given by the volume integral
$$
\mathbf{P}_\text{EM} = \frac{1}{4\pi c} \int d^3\mathbf{x} \ (\mathbf{E} \times \mathbf{B}) \ .
$$
We now must deal with the right-hand side. The goal is to extract from this term the divergence of something that we can identify as a momentum flux density. Since $\mathbf{g}$ is a vector, there will be one flux density vector per component of $\mathbf{g}$, or equivalently a rank-2 tensor for the entire vector, which we'll denote by $\mathbf{T}$.
...
Finally, we'll consider the remaining quantity that's often conserved in classical mechanics, angular momentum. Recall that the mechanical angular momentum $\mathbf{L}_\text{mech}$ is defined in terms of the ordinary linear momentum by
$$
\mathbf{L}_\text{mech} \equiv \mathbf{x} \times \mathbf{P}_\text{mech} \ .
$$
We can convert this formula into a statement about densities as well. If we let
$$
\mathbf{P}_\text{mech} = \int d^3 \mathbf{x} \ \mathbf{g}_\text{mech} \ ,
$$
we have
$$
\mathbf{L}_\text{mech} = \int d^3 \mathbf{x} \ (\mathbf{x}' \times \mathbf{g}_\text{mech})
$$

- Be more careful here. Follow Jackson + notes.
