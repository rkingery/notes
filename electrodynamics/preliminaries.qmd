...
- Convolution: $\mathcal{F}[f \ast g](\omega) = f(\omega) g(\omega)$.

As an example of using these properties let's calculate the Fourier transform of the Gaussian probability density
$$
...
In one sense, the delta function is just the Fourier transform of the constant $f(t)=1$. In another sense it's an infinite spike. Indeed, we can imagine defining the delta function differently as a limiting case of a Gaussian whose variance $\sigma^2$ goes to zero,
$$
