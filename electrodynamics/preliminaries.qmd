...
- Rewrite this section to emphasize that we'll only take Fourier transforms in spatial dimensions, not time dimensions.
- For time dimensions we'll need to take the Laplace transform, so you may need to mention that too.
- Try to hand-wave this section less, since the Fourier transform will be used a decent bit.
...
- Move the stuff that's essential only to conformal mappings to that section.
- Here be sure to cover the basics of complex variables, and *maybe* the residue theorem.
- Note the residue theorem won't be used until we get to electrodynamics, so it could be thrown in somewhere else, so we can cover complex analysis in a little more depth.
