...
In this chapter and the next, we will continue on with the subject of electrostatics by focusing on some of the more advanced aspects of the subject. Many of the things we cover in these two chapters will also apply to other areas of electromagnetism as well, like magnetostatics and electrodynamics, with minor modifications here and there that we'll touch on in future chapters. 
...
We will now introduce the idea of a *Green's Function*, which is a very useful and important concept in electromagnetism and many other field theories. We'll see that the Green's function provides a general formulation for solving Poisson's equation, and indeed for solving any linear PDE.
...
G(\mathbf{x} - \mathbf{x}') = \frac{1}{|\mathbf{x} - \mathbf{x}'|} \ .
$$
This, of course, is exactly what we wanted to prove.
...
One can check that the charge $Q$ on the surface of the sphere is given by $Q = q' + q'' = VR - \frac{R}{d} q$. The force between the conductor in the sphere is evidently given by
$$
\begin{align*}
\mathbf{F} &= q\bigg(\frac{q'}{|\mathbf{d} - \mathbf{d}'|^2} + \frac{VR}{|\mathbf{d}|^2}\bigg)\mathbf{e}_r \\ 
&= \frac{q}{d^2} \bigg(VR - \frac{qRd^3}{\big(d^2 - R^2\big)^2}\bigg) \mathbf{e}_r \ .
