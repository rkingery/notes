...
$$
In principle, this integral formula will work as long as the charge distribution is localized, there are no other sources of charge, and we've already specified in advance what the charge density $\rho(\mathbf{x}')$ actually is. Unfortunately, in practice there are other sources of charge present, and we often don't know what the charge density is. For example, there may be background conductors or other charged materials present that are too difficult to model, but we know the potential on those surfaces. We can model situations like this by introducing *boundary conditions*, while modeling the charge densities we care about directly.

The natural way to find the potential in the presence of arbitrary boundary conditions like this is to instead express the problem in terms of a partial differential equation (PDE) subject to a set of boundary conditions. We saw in the previous chapter that we can specify the potential via *Poisson's Equation*,
$$
\nabla^2 \phi = -4\pi\rho \ .
$$
This is a linear second-order PDE. To solve for the exact potential of a given problem, we need to match the PDE with a set of *boundary conditions*. For Poisson's equation we do that by specifying the value of either the potential or its normal derivative along some given *boundary surface*.

## Green's Functions

We will now introduce the idea of a *Green's Function*, which is a very useful and important concept in electromagnetism and many other field theories, as well as the theory of partial differential equations. We'll see that the Green's function provides a general formulation for solving Poisson's equation, and indeed for solving any linear PDE.
...
\end{align*}
$$
This proves that if we can find the Green's function, we can use it to find the potential for any given charge distribution by convolution.
...
$$
we could just skip ahead to the next section. However, we want to derive the Green's function yet another way, using the properties of the Fourier transform. We introduce this new method because in future chapters we'll want to find the Green's function for other PDEs in future chapters, where we can't rely on already-known formulas to help us.

Let's suppose that we wished to solve Poisson's equation for a unit point charge,
$$
\nabla^2 G(\boldsymbol{\xi}) = -4\pi \delta(\boldsymbol{\xi}) \ ,
$$
where $\boldsymbol{\xi} = \mathbf{x} - \mathbf{x}'$. We'll assume that both $G$ and its normal derivative go to zero as $|\boldsymbol{\xi}| \rightarrow \infty$, a physically realistic assumption.

Now, what we can do is take the Fourier transform of both sides,
$$
\mathcal{F} \big(\nabla^2 G(\boldsymbol{\xi})\big) = \mathcal{F} \big(-4\pi \delta(\boldsymbol{\xi})\big) \ .
$$
Let $\mathbf{k}$ denote the Fourier conjugate to $\boldsymbol{\xi}$ and $G(\mathbf{k})$ denote the Fourier transform of $G(\boldsymbol{\xi})$. Then the left-hand side becomes
$$
\mathcal{F} \big(\nabla^2 G(\boldsymbol{\xi})\big) = \int d^3 \boldsymbol{\xi} \ \nabla^2 G(\boldsymbol{\xi}) e^{-i \mathbf{k} \cdot \boldsymbol{\xi}} \ .
$$
Now, if we apply Green's second identity and require that $G$ and its normal derivative vanish at infinity, we get
$$
\begin{align*}
\mathcal{F} \big(\nabla^2 G(\boldsymbol{\xi})\big) &= \int d^3 \boldsymbol{\xi} \ \nabla^2 G(\boldsymbol{\xi}) e^{-i \mathbf{k} \cdot \boldsymbol{\xi}} \\
&= \int d^3 \boldsymbol{\xi} \ G(\boldsymbol{\xi}) \nabla^2 e^{-i \mathbf{k} \cdot \boldsymbol{\xi}} + 
\oint_\mathcal{S} da \ \bigg[e^{-i \mathbf{k} \cdot \boldsymbol{\xi}} \frac{\partial G}{\partial n} - G \frac{\partial}{\partial n} e^{-i \mathbf{k} \cdot \boldsymbol{\xi}} \bigg] \\
&= -|\mathbf{k}|^2 \int d^3 \boldsymbol{\xi} \ G(\boldsymbol{\xi}) e^{-i \mathbf{k} \cdot \boldsymbol{\xi}} \\
&= -|\mathbf{k}|^2 G(\mathbf{k}) \ .
\end{align*}
$$
This result is important enough that it's useful to remember: For any well-behaved scalar field $f(\mathbf{x})$, the Fourier transform of its Laplacian is given by $\nabla^2 f(\mathbf{x}) \leftrightarrow -|\mathbf{k}|^2 f(\mathbf{k})$.

Now, for the right-hand side we can use the definition of the delta function to write
$$
\mathcal{F} \big(-4\pi \delta(\boldsymbol{\xi})\big) = -4\pi \int d^3 \boldsymbol{\xi} \ \delta(\boldsymbol{\xi}) e^{i \mathbf{k} \cdot \boldsymbol{\xi}} = -4\pi \ .
$$
Putting these together, the Fourier dual of the original Green's function PDE is just
$$
-|\mathbf{k}|^2 G(\mathbf{k}) = -4\pi \ .
$$
This means the Fourier transform of the Green's function is just given by
$$
G(\mathbf{k}) = \frac{4\pi}{|\mathbf{k}|^2} \ .
$$
To get back $G(\boldsymbol{\xi})$ we now need to take the inverse Fourier transform of both sides,
$$
G(\boldsymbol{\xi}) = \mathcal{F}^{-1} G(\mathbf{k}) = \mathcal{F}^{-1} \bigg(\frac{4\pi}{|\mathbf{k}|^2}\bigg) \ .
$$
Using the definition of the inverse Fourier transform, we evidently have
$$
G(\boldsymbol{\xi}) = \int \frac{d^3 \mathbf{k}}{(2\pi)^3} \ G(\mathbf{k}) e^{-i \mathbf{k} \cdot \boldsymbol{\xi}} = 4\pi \int \frac{d^3 \mathbf{k}}{(2\pi)^3} \ \frac{e^{-i \mathbf{k} \cdot \boldsymbol{\xi}}}{|\mathbf{k}|^2} \ .
$$
We now need to evaluate this integral over $\mathbf{k}$-space the hard way. We'll work in spherical coordinates $(k,\theta_k,\phi_k)$. This means the volume element reduces to
$$
d^3 \mathbf{k} = k^2 \sin\theta_k dk d\theta_k d\phi_k \ .
$$
Now, suppose without loss of generality that $\boldsymbol{\xi}$ is oriented in the $k_z$-direction, so we can write $\mathbf{k} \cdot \boldsymbol{\xi} = \xi k \cos\theta_k$, where $\xi = \boldsymbol{\xi}$. Then we have
$$
G(\boldsymbol{\xi}) = 4\pi \int \frac{k^2 \sin\theta_k dk d\theta_k d\phi_k}{(2\pi)^3} \ \frac{e^{-i k \xi \cos\theta_k}}{k^2} = \frac{1}{\pi} \int dk d\theta_k \ \sin\theta_k \ e^{-i k \xi \cos\theta_k} \ .
$$
To evaluate the integral over $\theta_k$, we'll again make the common substitution $\mu = \cos\theta_k$ to get
$$
\begin{align*}
G(\mathbf{x}) &= \frac{1}{\pi} \int_0^\infty dk \int_{-1}^1 d\mu \ e^{-i k \xi \mu} \\
&= \frac{1}{\pi} \int_0^\infty dk \ \frac{e^{-ik\xi} - e^{ik\xi}}{-ik\xi} \\
&= \frac{2}{\pi} \int_0^\infty dk \ \frac{\sin k\xi}{k\xi} \ ,
\end{align*}
$$
What remains is an integral over $k$. We'll make another substitution $u = k\xi$ to simplify the integral somewhat,
$$
G(\boldsymbol{\xi}) = \frac{2}{\pi \xi} \int_0^\infty du \ \frac{\sin u}{u} \ .
$$
The remaining integral is a well-known integral known as a *Dirichlet integral*. These kinds of integrals can be evaluated any number of ways. We'll use a technique popularized by Feynman known as the *Feynman trick*. 

The idea is to introduce a dummy variable $t$ into the integrand and use differentiation under the integral sign to evaluate the integral in terms of $t$, then set $t=0$ to recover the original integral. Consider the integral
$$
...
$$
G(\boldsymbol{\xi}) = \frac{2}{\pi \xi} \cdot \frac{\pi}{2} = \frac{1}{\xi} = \frac{1}{|\boldsymbol{\xi}|} \ .
$$
That is,
$$
G(\mathbf{x} - \mathbf{x}') = \frac{1}{|\mathbf{x} - \mathbf{x}'|} \ .
...
We'll now derive a formal solution to Poisson's equation when boundary conditions are present. To do so we'll make use of Green's second identity. Recall that Green's second identity says that for two scalar fields $\psi(\mathbf{x}')$ and $\phi(\mathbf{x}')$, we have
...
$$
where $F$ is any function that satisfies Laplace's equation subject to the given boundary conditions, i.e.
$$
\nabla^2 F(\mathbf{x} - \mathbf{x}') = 0 \ .
$$
Such a solution without a source term is usually called a *homogeneous solution*.

Intuitively, we can think of $F$ as the potential of any charge distributions lying beyond the boundary surface. Our goal in this section however isn't to find $F$, but to formally show that generalized Green's functions give us formal solutions for the potential that aren't overdetermined.
...
### Green's Function in Other Dimensions

For the special case of the Laplacian operator, there's an easier way to find the (unbounded) Green's function that also happens to work just as easily for any number of dimensions. Let's again consider Poisson's equation for the Green's function free from boundary conditions, but this time we'll assume $\mathbf{x}$ and $\mathbf{x}'$ are vectors in some arbitrary $n$-dimensional space. If we ignore the factor of $4\pi$ from before, we have
$$
...
$$
Notice that the first term is just the unbounded Green's function. The second term is the homogeneous function $F$ that satisfies the boundary conditions, i.e.
$$
...
$$
Again, the first term is just the unbounded Green's function. The second term is the homogeneous function $F$ that satisfies the boundary conditions,
$$
...
$$
The first term is the usual unbounded part of the Green's function, and what remains is the homogeneous function $F$ that satisfies Laplace's equation subject to the boundary conditions, as one can check.
