...
In a series of experiments, Ampere established that two parallel current carrying wires exert a force on each other. In particular, he observed that the when two parallel wires have a current running through each of them that they tend to attract or repel each other, and the force per unit length of wire is proportional to the product of their currents and inversely proportional to the distance between the wires. If the currents are aligned the wires repel, and if they're anti-aligned the wires attract.

More formally, suppose we have two parallel wires with currents $I_1$ and $I_2$ each of length $L$ separated by some distance $d \ll L$. Then the force strength $F$ per unit length $L$ between the wires is given by the relation
$$
\frac{F}{L} = 2k_m \frac{I_1 I_2}{d} \ .
$$
This is empirical relation is now known as *Ampere's Current Law*. The force per unit length $F/L$ will always be in the direction perpendicular to the wires and along the axis of their separation distance. If the two currents run in the same direction, $I_1 I_2$ will be *positive*, which means the wires will *repel*. If they run in opposite directions, $I_1 I_2$ will be *negative*, which means they'll *attract*.
...
It's worth asking whether Ampere's force law could in fact be an electric phenomenon, not something new.

- Finish this part. Will need to research first.
...
We'll now present the important Biot-Savart law, which is essentially the equivalent of Coulomb's law for magnetostatics. Biot and Savart were able to use the results of Ampere and others to find a mathematical relationship between the current and the B-field in a magnetostatic setting. As we'll see, this law looks somewhat more cumbersome than Coulomb's law. This is partly due to the fact that we have to work from the outset with a continuous current distribution for the reasons we described before.
...
$$
But this can't be true, since the Biot-Savart law already assumes a steady current, and point charges can't have a steady current. The only way to calculate the B-field of a point charge is to use electrodynamics. We'll come back to this in a later chapter.
