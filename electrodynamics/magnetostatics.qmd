...
It's tempting to ask whether we can use the Biot-Savart law to calculate the B-field generated by a moving point charge. If we naively attempted to apply the Biot-Savart law to a moving point charge $q$ of velocity $\mathbf{v}$, we'd find a B-field
$$
\mathbf{B}(\mathbf{x}) = q \frac{\mathbf{v}}{c} \times \frac{\mathbf{x} - \mathbf{v}t}{|\mathbf{x} - \mathbf{v}t|^3} \ .
$$
But this can't be true, since the Biot-Savart law requires that B-field only be a function of position and not time. The only way the B-field can be time-independent is if the steady current condition is imposed, which means the Biot-Savart law implicitly assumes the steady current condition is true. We thus cannot use the Biot-Savart law on point charges, at least without there being so many of them that we can meaningfully impose the steady current condition. To deal with point charges we'll necessarily need electrodynamics, where we allow the electric and magnetic fields to both be time-dependent.
...
Since $\rho$ and $\mathbf{J}/c$ have the same units, this evidently means the scalar potential and the vector potential also have the same units in Gaussian units. The vector potential evidently carries units of $\text{erg} / \text{esu}$, or equivalently units of $\text{G} \cdot \text{cm}$.

### Gauge Transformations

In electrostatics, we saw that we could add any constant to the scalar potential and it wouldn't affect the physics, since in the end the E-field is the gradient of the scalar potential. A similar concept exists in magnetostatics, except it's more subtle. What physically matters is the B-field, since it's the B-field that gives rise to the forces. 
...
\mathbf{B} = \nabla \times \mathbf{A} = \nabla \times \mathbf{A}' \ .
$$
Thus, any transformation $\mathbf{A} \to \mathbf{A}'$ that adds the gradient of any gauge potential $\chi$ won't alter the underlying physics. We call any transformation of this form a *gauge transformation*. This means that given any vector potential $\mathbf{A}$, we can always do a gauge transformation without altering the B-field $\mathbf{B}$. In particular, this means any two given vector potentials may look very different, yet still be *gauge equivalent*, in the sense that they give rise to the same B-field. This is very different to what we saw in electrostatics, where the scalar potential of a given E-field could only differ by an additive constant.
...
- Derive surface boundary conditions for $B$ and $A$
...
$$
where $\mathcal{V}$ is the enclosed volume of the current distribution. If we plug in the Taylor expansion above into the force law and group terms, we have
$$
\begin{align*}
...
\end{align*}
$$
Now, the first term will vanish. Indeed, for a localized distribution of steady current we've already shown that
$$
\int_\mathcal{V} d^3\mathbf{x} \ \mathbf{J}(\mathbf{x}) = \mathbf{0} \ .
$$
We'll thus focus on the second term. Recall how we derived the magnetic monopole moment before. We showed that
$$
\frac{1}{c} \mathbf{x} \cdot \int d^3\mathbf{x}' \ \mathbf{x}' J_i = (\mathbf{m} \times \mathbf{x})_i \ .
$$
In that derivation there was nothing special about $\mathbf{x}$. We could replace it with any vector we like. Notice if we replace $\mathbf{x} \to \nabla B_k$ and $\mathbf{x}' \to \mathbf{x}$ in the above relation, we get
$$
\frac{1}{c} \bigg[\int d^3\mathbf{x} \ J_i \mathbf{x}\bigg] \cdot \nabla B_k = (\mathbf{m} \times \nabla B_k)_i \ .
$$
On the left-hand side is just the second term. We thus have
$$
\mathbf{F} = (\mathbf{m} \times \nabla) \times \mathbf{B}
$$


- This still isn't quite right. The final step is true, but the derivation isn't. Dig deeper here. It's a huge pain in the ass.

$$
F_i \approx \frac{1}{c} \int_\mathcal{V} d^3\mathbf{x} \ \varepsilon_{ijk} J_j (\mathbf{x} \cdot \nabla) B_k^{\text{ext}} \ .
$$

- Continue on with lectures. Don't feel like it tonight.
