...
Suppose a hollow sphere of radius $R$ is rotating with some constant angular velocity $\omega$ about some axis. On the sphere is a uniform surface charge density $\sigma$. We'd like to calculate the vector potential and magnetic field of this rotating sphere.
...
$$
Now, by definition $\mathbf{K}(\mathbf{x}') = \sigma \mathbf{v}'$, and since $|\mathbf{x}'| = R$ we have
$$
|\mathbf{x} - \mathbf{x}'| = \sqrt{r^2 + R^2 - 2 \mathbf{x} \cdot \mathbf{x}'} \ .
$$
At this point we need to choose a coordinate system to do the integration. Obviously spherical coordinates makes the most sense, but rather than orient the angular velocity vector $\boldsymbol{\omega}$ along the $z$-axis like we normally would, it turns out to be easier to orient the coordinate system so that the field point $\mathbf{x}$ is oriented along the $z$-axis and $\boldsymbol{\omega}$ is in the $xy$-plane. This means we have $\mathbf{x} \cdot \mathbf{x}' = rR\cos\theta'$, and so
$$
|\mathbf{x} - \mathbf{x}'| = \sqrt{r^2 + R^2 -2rR\cos\theta'} \ .
$$
The velocity vector $\mathbf{v}'$ is given by the cross product $\boldsymbol{\omega} \times \mathbf{x}'$, which is evidently
$$
\mathbf{v}' = \boldsymbol{\omega} \times \mathbf{x}' = \omega R \mathbf{e}_\varrho \times \mathbf{e}_{r'}
$$
This means the surface current density is then $\mathbf{K}(\mathbf{x}') = R\sigma\omega \cdot$, and so the vector potential can be written
$$
...
$$
\mathbf{g}(\mathbf{x}) = G \int d^3 \mathbf{x}' \ \rho(\mathbf{x}') \frac{\mathbf{x} - \mathbf{x}'}{|\mathbf{x} - \mathbf{x}'|^3}
$$

$$
\mathbf{g}(\mathbf{x}) = -\nabla \phi(\mathbf{x}) \quad , \quad \phi(\mathbf{x}) = G \int d^3\mathbf{x}' \ \frac{\rho(\mathbf{x}')}{|\mathbf{x} - \mathbf{x}'|}
$$

$$
M = \int d^3\mathbf{x}' \ \rho(\mathbf{x}') \quad , \quad \mathbf{R} = \frac{1}{M} \int d^3\mathbf{x}' \ \rho(\mathbf{x}') \mathbf{x}'
$$

$$
\phi(\mathbf{x}) = G\bigg[\frac{M}{r} + \frac{M\mathbf{R} \cdot \mathbf{x}}{r^2} + \frac{\mathbf{x} \cdot \mathbf{M} \cdot \mathbf{x}}{2r^3} + \cdots\bigg]
$$

$$
\mathbf{M} = \int d^3\mathbf{x}' \ \rho(\mathbf{x}') [3\mathbf{x}' \otimes \mathbf{x}' - r'^2 \mathbf{1}]
$$

$$
\mathbf{I} = \int d^3\mathbf{x}' \ \rho(\mathbf{x}') [r'^2 \mathbf{1} - \mathbf{x}' \otimes \mathbf{x}']
$$
