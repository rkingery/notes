...
\end{align*}
$$
Now, the first term will vanish. Indeed, for a localized current distribution we've already shown that
$$
\int_\mathcal{V} d^3\mathbf{x} \ \mathbf{J}(\mathbf{x}) = \mathbf{0} \ .
$$
We'll thus focus on the second term. At this point it's helpful to work in index notation. We have
$$
F_i \approx \frac{1}{c} \int_\mathcal{V} d^3\mathbf{x} \ \varepsilon_{ijk} J_j x_\ell \partial_\ell B_k^{\text{ext}} \ .
$$
