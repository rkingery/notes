...
TODO: TWO STATE SYSTEMS, EINSTEIN SOLID
...
$$
Despite appearing to just be a normalization constant the partition function turns out to be very important to statistical mechanics. It essentially encodes all the statistical mechanical information contained in the system. To see why it's helpful to re-write the partition function as an integral (or sum) over all possible system energies $E$. To do that multiple microstates can have the same energy. That means we need to multiply the integrand by a multiplicity $\Omega(E)$. We thus have
$$
...
$$
where $Z(\beta,J)$ is again a normalization constant. It's called the **Gibbs partition function**, and is given by integrating over all microstates $\boldsymbol{\mu}$ and displacements $X$,
$$
Z(\beta,J) \equiv \int dX \ d \boldsymbol{\mu} \ e^{-\beta \big(H(\boldsymbol{\mu})-J \cdot X\big)}.
$$
By factoring the dependences on $\boldsymbol{\mu}$ and $X$ we can write the Gibbs partition function in terms of the canonical partition function $Z(\beta)$ as
$$
Z(\beta,J) = \int dX \ e^{-\beta J \cdot X} \ Z(\beta).
$$
Note the abuse of notation. We're using $Z$ to refer to both partition functions, and letting the functional dependence specify which we're talking about. When it's obvious which partition function we're referring to we'll often drop the dependence and just write $Z$.
...
$$
In particular, all cumulants of $X$ are extensive. This means the variance is proportional to $\langle X \rangle$, which means the fluctuations in $X$ go like $\delta X = \sqrt{\langle X \rangle}$. This means we can again assert that $X = X^* = \langle X \rangle$ when $N$ is really large.

We can relate the partition function to the free energy by observing
$$
...
$$
where $Z(\beta,\mu)$ is another normalization constant gotten by integrating over all possible $\boldsymbol{\mu}$ and summing over all possible $N$. This is called the **grand canonical partition function**, given by
$$
...
$$
The dimensionless variable $f \equiv \beta\mu = \frac{\mu}{k_B T}$ is called the **fugacity**. It will turn out to be important in quantum statistical mechanics since its size says something about the limiting behaviors of the energy and entropy.
...
$$
In particular, all cumulants of $N$ are proportional to $\langle N \rangle$. In particular, this means the variance is proportional to $\langle N \rangle$, which means the fluctuations in $N$ go like $\delta N = \sqrt{\langle N \rangle}$. By the same usual logic, this means we can assert that $N = N^* = \langle N \rangle$ provided $N^*$ is large, which will typically be the case in thermodynamics.
...
$$
We can get the mean volume $\langle V \rangle$ by differentiating both sides with respect to $-\beta P$,
$$
\langle V \rangle = \frac{\partial}{\partial (-\beta P)} \log Z(\beta,P) = \frac{N}{\beta P} = \frac{Nk_B T}{P}.
$$
This is of course the usual equation of state, with $P\langle V \rangle = N k_B T$. We can easily find the enthalpy as well,
$$
H = -\frac{\partial}{\partial \beta} \log Z(\beta,P) = \frac{3N}{2\beta} + \frac{N}{\beta} = \frac{5}{2} N k_B T.
$$
Since $H = E + P\langle V \rangle$, we can immediately read off the usual formula for energy, $E = \frac{3}{2} N k_B T$.
...
\end{align*}
$$
This means $\log Z(\beta,\mu)$ is just
$$
\log Z(\beta,\mu) = \frac{Ve^{\beta\mu}}{h^3} \bigg(\frac{2\pi m}{\beta}\bigg)^{3/2}.
$$
It's helpful to first find $\langle N \rangle$. We have
$$
...
$$
This means all cumulants of $N$ will be $\log Z(\beta,\mu)$ as well. Recall this fact implies that $N$ must be Poisson distributed, with
$$
p(N) = \frac{\langle N \rangle^N e^{-\langle N \rangle}}{N!}.
$$
The grand potential is evidently just $\mathcal{G} = -\langle N \rangle k_B T$. But by extensivity $\mathcal{G} = -PV$. We thus again have the ideal gas law,
$$
...
$$
Cancelling $\mu \langle N \rangle$ from both sides, we again get $E = \frac{3}{2} \langle N \rangle k_B T$. Finally, if we like we can solve for the chemical potential by inverting the formula for $\langle N \rangle$. As expected, we have
$$
