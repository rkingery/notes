...
$$
In particular, all cumulants of $N$ are proportional to $\langle N \rangle$. In particular, this means the variance is proportional to $\langle N \rangle$, which means the fluctuations in $N$ go like $\sigma_N = \sqrt{\langle N \rangle}$. By the same usual logic, this means we can assert that $N = N^* = \langle N \rangle$ provided $N^*$ is large, which will typically be the case in thermodynamics.
