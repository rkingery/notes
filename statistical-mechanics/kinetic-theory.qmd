...
$$
\boxed{\Eta(t) \equiv \int d^3 \mathbf{x} \ d^3 \mathbf{p} \ f(\mathbf{x}, \mathbf{p}, t) \log f(\mathbf{x}, \mathbf{p}, t)}
$$
...
$$
\begin{align*}
\frac{d\Eta}{dt} &= \int d^3 \mathbf{x} \ d^3 \mathbf{p} \ \frac{\partial}{\partial t}(f \log f) \\
...
$$
\begin{align*}
\frac{d\Eta}{dt} &= \int d^3 \mathbf{x} \ d^3 \mathbf{p} \ \log f \ C[f] \\
...
$$
\mathbf{F}_{\text{eff}} = q\mathbf{E} +  q\frac{\mathbf{v}}{c} \times \mathbf{B}.
$$
...
$$
This rule is sometimes called the law of *detailed balance*. Detailed balance evidently says that the sum $\log f(\mathbf{x}, \mathbf{p}_1) + \log f(\mathbf{x},\mathbf{p}_2)$ must be conserved during collisions. In an elastic collision we require that particle number, momentum and kinetic energy be conserved,
$$
\begin{align*}
1 + 1 &= 1' + 1',\\
...
If desired, one can calculate the entropy too by using $S \equiv -k_B \mathrm{H}$ and doing the integral for $\mathrm{H}$. The result will be the Sakur-Tetrode equation for a monoatomic ideal gas, up to an additive constant. In fact, we could calculate the entropy and use thermodynamic relations to get the energy and pressure more straightforwardly. This suggests a more statistical approach to calculating thermodynamic relations that we'll make heavy use of in statistical mechanics.
...
$$
This is the distribution for the *velocity* of a gas in the rest frame, not the speed. It's evidently a mean-zero Gaussian distribution with variance $\frac{k_B T}{m}$. The ratio $v_{RMS}^2 \equiv \frac{k_B T}{m}$ has dimensions of velocity squared. That velocity $v_{RMS}$ is amply referred to as the root mean square (RMS) velocity. We've actually seen it already. It's the velocity one gets from setting the kinetic energy equal to the mean thermal energy $k_B T$. Practically all velocities of interest in thermodynamics are on the order of the RMS velocity.
...
$$
\langle v \rangle = 2\sqrt{\frac{2}{\pi}} \sqrt{\frac{k_B T}{m}} \approx 1.6 \ v_{RMS}.
$$
## Conservation Laws
...
$$
\boxed{\big\langle \chi(\mathbf{x},t) \big\rangle_{\text{col}} \equiv \frac{1}{n(\mathbf{x}, t)} \int d^3 \mathbf{p} \ f(\mathbf{x}, \mathbf{p}, t) \ \chi(\mathbf{x}, \mathbf{p}, t)} \ .
$$
...
$$
\boxed{\frac{\partial}{\partial t} n\big\langle \chi \big\rangle_{\text{col}} + \frac{\partial}{\partial \mathbf{x}} \cdot n\bigg\langle \frac{\mathbf{p}}{m} \chi \bigg\rangle_{\text{col}} - \ n \bigg\langle \frac{\partial \chi}{\partial t} \bigg\rangle_{\text{col}} - \ n \bigg\langle \frac{\mathbf{p}}{m} \cdot\frac{\partial \chi}{\partial \mathbf{x}}\bigg\rangle_{\text{col}} - n \mathbf{F} \cdot \bigg\langle \frac{\partial \chi}{\partial \mathbf{p}}\bigg\rangle_{\text{col}} = 0 } \ .
$$
...
$$
\frac{\partial n}{\partial t} + \frac{\partial}{\partial \mathbf{x}} n\bigg\langle \frac{\mathbf{p}}{m} \bigg\rangle_{\text{col}} = 0.
$$
...
$$
P_{\alpha\beta} \equiv nm \big\langle c_\alpha c_\beta \big\rangle_c.
$$
...
In the zeroth order case we're assuming $\tau_{\text{ext}} \rightarrow \infty$. Call that *zeroth order density* $f^0$. Then we have $C[f^0] = 0$. We already saw using detailed balance that such a distribution is just a Gaussian in terms of the momenta or velocity. If we write
$$
...
$$
\mathbf{P} = nm \big\langle \mathbf{c} \otimes \mathbf{c} \big\rangle_c = nm \frac{k_B T}{m} \ \mathbf{I} = nk_B T \ \mathbf{I}.
$$
...
\end{pmatrix}.
$$
Note the block notation being used. This is really a $5 \times 5$ matrix multiplying a size $5$ vector. There are thus $5$ distinct normal modes that satisfy this equation. The solutions are rather messy. Suffice it to say that:

- Two of the modes represent *shearing modes*, e.g. flows of the form $\mathbf{u} = u_x(y,t) \mathbf{e}_x$. That is, the velocities vary perpendicularly to their direction of motion, creating a shearing effect. This means that the *transverse flows* can't relax to equilibrium. They continue forever.
- One of the modes is a *zero mode*, i.e. $\omega = 0$. This describes a ** with uniform pressure $P = n k_B T$. This insures the product $nT$ is constant, which means the fluid can't start moving due to pressure variations alone. The normal modes are $\tilde\nu = n_0$, $\tilde\theta = -T_0$, and $\mathbf{\tilde u} = \mathbf{0}$, all constant.
- The final two modes are *sound modes*. These are normal modes of the form $\tilde\nu = n_0 |\mathbf{k}|$, $\tilde\theta = \frac{2}{3} T_0 |\mathbf{k}|$, and $\mathbf{\tilde u} = \pm v_s |\mathbf{k}|$. Here $v_s^2 \equiv \frac{5k_B T_0}{3m}$ represents the *speed of sound* of the gas. All of these modes imply that the quantities all propagate out as an undamped wave in the $\mathbf{k}$ direction. Since both the density and temperature variations are adiabatic, these *longitudinal flows* won't relax to equilibrium either.
...
Also worth noticing is that the first-order piece contains terms proportional to both $c$ and $c^3$, which means the first and third moments no longer vanish. Together, these will imply the existence of a non-zero heat flux vector and a non-diagonal pressure tensor. So
