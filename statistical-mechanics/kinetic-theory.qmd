...
If desired, one can calculate the entropy too by using $S \equiv -k_B \mathrm{H}$ and doing the integral for $\mathrm{H}$. The result will be (up to an additive constant) the entropy for a monoatomic ideal gas. This is called the **Sakur-Tetrode equation**,
$$
\boxed{S = Nk_B \log\bigg[\frac{V}{N} \bigg(\frac{mE}{3\pi \hbar^2 N}\bigg)^{3/2} \bigg] + \frac{5}{2} Nk_B} \ .
$$
In fact, we could calculate the entropy and use thermodynamic relations to get the energy and pressure more straightforwardly.
...
Also worth noticing is that the first-order correction now contains terms proportional to both $c$ and $c^3$, which means the first and third moments no longer vanish. Together, these will imply the existence of a non-zero heat flux vector and a non-diagonal pressure tensor. 
