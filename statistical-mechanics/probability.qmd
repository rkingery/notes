...
$$
\begin{align*}
&\max_{p(x)} S[p(x)] =  \max_{p(x)} \bigg(- \sum_x p(x) \log p(x) \bigg) \\
&\text{subject to} \ \sum_x p(x) = 1, \ \text{and} \ g(x) = 0. \\
\end{align*}
$$
...
$$
L(p(x),\lambda) \equiv - \sum_x p(x) \log p(x) - \alpha \bigg(\sum_x p(x) - 1\bigg) - \beta \cdot g(x).
$$
...
$$
\begin{align*}
&\max_{p(x)} \bigg(- \int dx \ p(x) \log p(x) \bigg) \\
&\text{subject to} \ \int dx \ p(x) = 1, \\ 
&\text{and} \int dx \ x p(x) = \mu, \ \ \int dx \ x^2 p(x) - \mu^2 = \sigma^2. \\
\end{align*}
$$
The Lagrange multiplier function is then
$$
\begin{align*}
L(p(x),\alpha,\beta,\gamma) = -&\bigg(\int dx \ p(x) \log p(x) \bigg) - \alpha\bigg(\int dx \ p(x) - 1\bigg) \\
- \beta&\bigg(\int dx \ x p(x) - \mu\bigg) - \gamma \bigg(\int dx \ x^2 p(x) - \mu^2 - \sigma^2\bigg).
\end{align*}
$$
...
&= -\int dx \delta p \ \bigg[\log p + 1 + \alpha + \beta x + \gamma(x^2 - 2\mu x) \bigg] \\
&\equiv 0.
\end{align*}
$$
...
$$
where $Z$ must satisfy the relation
$$
Z \equiv \sum_{i=0}^\infty e^{-\beta E_i}.
$$
We'll see later that $Z$ is very important to statistical mechanics. It's called the **partition function**. In that case, $E_j$ represents the energy of the system in the $j$<sup>th</sup> state and $E$ represents the average internal energy of the system, i.e. the energy that satisfies the first law of thermodynamics.

More generally, we could imagine $E(x)$ being a vector-valued function, in which case we'd get
$$
p_j = \frac{1}{Z} e^{-\beta \cdot E_j},
$$
where each $\beta$ and $E_j$ is now a vector.  The partition function would similarly change to be
$$
Z \equiv \sum_{i=0}^\infty e^{-\beta \cdot E_i}.
$$
