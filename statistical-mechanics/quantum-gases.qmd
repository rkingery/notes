...
&= \frac{(\log z)^s}{s!} \bigg[1 + \zeta_2 \frac{s(s-1)}{(\log z)^2} + \frac{7\zeta(4)}{4}\frac{s(s-1)(s-2)(s-3)}{(\log z)^4} + O\big((\log z)^{-6}\big) \bigg] \\
...
The tendency of a material to respond to an external magnetic field $\mathbf{B}$ is captured via its *magnetization* vector $\mathbf{M}$. Its magnitude $M$ is roughly speaking the average magnetic dipole moment per unit volume, which can be more usefully related to the difference between densities of up-spin to down-spin unpaired electrons by
$$
M \equiv \frac{\partial \mu}{\partial V} = \mu_B \frac{N_+ - N_-}{V} \ ,
$$
where $\mu_B \equiv \frac{e\hbar}{2m_ec} \approx 5.8 \cdot 10^{-9} \ \text{eV} \cdot \text{G}^{-1}$ is a constant known as the *Bohr magneton*. The magnetization depends on the strength of the external field. If $\mathbf{B} = \mathbf{0}$ then $\mathbf{M} = \mathbf{0}$ as well. We can relate the two via a constituency relation whose form depends both on the material as well as the strength of the external field. In the simplest case where the material is *isotropic* and $\mathbf{B}$ is sufficiently weak, $\mathbf{M}$ will be approximately linear in the auxiliary field $\mathbf{H} \equiv \mathbf{B} - 4\pi\mathbf{M}$, with
$$
\mathbf{M} \approx \chi \mathbf{H} \ ,
$$
where $\chi$ is a proportionality constant known as the *magnetic susceptibility*. Taking the differential of both sides and using the weak field assumption, we can also express $\chi$ more conveniently for our purposes via the formula
$$
\chi = \frac{\partial M}{\partial H} \approx \frac{\partial M}{\partial B} \bigg |_{B=0} \ .
$$
We also used the fact that for paramagnetic materials $\chi \ll 1$, so we can write $\frac{\chi}{1+4\pi\chi} \approx \chi$. The susceptibility $\chi$ expresses essentially all of the material-specific properties that contribute to the magnetization. It's thus useful to study its properties, for instance its dependence on thermodynamic variables like temperature or density. 

For paramagnetic materials, the energy per electron can be expressed by the single-particle Hamiltonian
$$
H_1 = \frac{(\mathbf{p}-\frac{e}{c}\mathbf{A})^2}{2m} + \boldsymbol{\mu} \cdot \mathbf{B} \approx \frac{\mathbf{p}^2}{2m} + \mu_B \boldsymbol{\sigma} \cdot \mathbf{B} \ ,
$$
where $m=m_e$ is the mass of the electron. Provided the external field is sufficiently weak and slowly varying we can make the approximation $\mathbf{p}-\frac{e}{c}\mathbf{A} \approx \mathbf{p}$ and recover the classical kinetic energy term. It's not too hard to see that the eigenstates of the $\boldsymbol{\sigma} \cdot \mathbf{B}$ operator alone are two states $|n_{\pm}\rangle$ with energies $\varepsilon_{\pm} = \pm B$. However, we need to be careful that we diagonalize the *joint* states $|n_{\mathbf{k}, \pm}\rangle$. In terms of these joint states, the energy eigenvalues would just be
$$
...
&= \prod_\mathbf{k} \bigg(1 + e^{-\beta \big(\varepsilon_{\mathbf{k}, +} - \mu\big)}\bigg) \bigg(1 + e^{-\beta \big(\varepsilon_{\mathbf{k}, -} - \mu\big)}\bigg) \\
&= \prod_\mathbf{k} \bigg(1 + e^{-\beta \big(\varepsilon_{\mathbf{k}} - \mu + \mu_B B\big)}\bigg) \prod_\mathbf{k} \bigg(1 + e^{-\beta \big(\varepsilon_{\mathbf{k}} - \mu - \mu_B B\big)}\bigg) \\
&\equiv \mathcal{Z}_0(\mu_+) \mathcal{Z}_0(\mu_-) \ .
\end{align*}
$$

Here we've for convenience defined $\mathcal{Z}_0(\mu)$ to be the grand partition function for the free Fermi gas from before. We'll also define $\mu_\pm \equiv \mu \mp \mu_B B$ to be a sort of effective chemical potential, and $z_\pm \equiv e^{\beta\mu_\pm}$ to be an effective fugacity. From here, we can take the logarithm and simplify by again using the density of states and the same substitutions to get
$$
\begin{align*}
\log \mathcal{Z} &= \log \mathcal{Z}_0(\mu_+) + \log \mathcal{Z}_0(\mu_-) \\
&\approx \frac{gV}{(2\pi)^3} \int d^3 \mathbf{k} \ \bigg[\log \bigg(1 + z e^{-\frac{\beta\hbar^2}{2m} \mathbf{k}^2} e^{-\beta \mu_B B}\bigg) + \log \bigg(1 + z e^{-\frac{\beta\hbar^2}{2m} \mathbf{k}^2} e^{\beta \mu_B B}\bigg)\bigg] \\
&= \frac{gV}{\lambda_T^3} \frac{1}{(3/2)!} \int_0^\infty dx \ \bigg[\frac{x^{3/2}}{z_{+}^{-1} e^x + 1} + \frac{x^{3/2}}{z_{-}^{-1} e^x + 1}\bigg] \\
&= \frac{gV}{\lambda_T^3} \big[f_{5/2}^{-}\big(z_+\big) + f_{5/2}^{-}\big(z_-\big)\big] \ .
\end{align*}
$$
From here we can proceed to calculate the mean up-spin and down-spin densities $n_\pm = \frac{N_\pm}{V}$. Using the differentiation ladder relation for polylogarithms, we just have
$$
n_\pm = \frac{1}{V} \frac{\partial \log\mathcal{Z}}{\partial (\beta\mu_\pm)} = \frac{z_\pm}{V} \frac{\partial \log\mathcal{Z}}{\partial z_\pm} = \frac{g}{\lambda_T^3} f_{3/2}^{-}\big(z_\pm\big) \ .
$$
This means the magnetization $M$ is just given by
$$
M = \frac{g}{\lambda_T^3} \big[f_{3/2}^{-}\big(z_+\big) + f_{3/2}^{-}\big(z_-\big)\big] \ ,
$$
from which we can conclude the susceptibility $\chi$ is given by
$$
\chi = \frac{\partial M}{\partial B} \bigg |_{B=0} = \frac{2g\beta\mu_B^2}{\lambda_T^3} f_{1/2}^{-}(z) \ .
$$
...
$$
N = N_+ + N_- = \frac{gV}{\lambda_T^3} \big[f_{3/2}^{-}\big(z_+\big) + f_{3/2}^{-}\big(z_-\big)\big] \ .
$$
At high temperatures we can use the fact that $f_{1/2}^{-}(z) \approx z$ to get $N \approx \frac{2gV}{\lambda_T^3} z$ and hence
$$
...
In the low temperature limit we use the Sommerfeld expansion in the weak field limit to write $f_s^{-}(z_\pm) \approx \frac{\beta^s\mu^s}{s!}$. From here, we can express $N \approx \frac{2gV}{(3/2)!} \frac{\beta^{3/2}}{\lambda_T^3} \varepsilon_F^{3/2}$ and plug this back into $\chi$ to get the following relation near zero temperature,
$$
...
We'll now consider the case of the degenerate Bose gas at low temperatures. As we'll see, bosons at low temperatures behave quite differently from fermions. The difference between the two gases comes down entirely to the mathematical differences between the functions $f_s^{+}(z)$ and $f_s^{-}(z)$. For fermions $f_s^{-}(z)$ is defined for all positive $z$, meaning $n, E, P$ are well-defined for all $z$ as well. For bosons though, $f_s^{+}(z)$ is only well-defined when $|z| < 1$. This means we have to think more carefully about what we mean by the variables $n, E, P$ at low temperatures.

Since $T=0$ corresponds to $z=1$ and we know $f_s^{+}(1) = \zeta_s$, we expect at zero temperature to have
$$
n = \frac{N}{V} = \frac{g}{\lambda_T^3} \zeta_{3/2} \approx 2.61 \frac{g}{\lambda_T^3} \ .
$$
